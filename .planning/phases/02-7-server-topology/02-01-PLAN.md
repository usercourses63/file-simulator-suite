...
must_haves:
  truths:
    ...
    - "Each NAS server can have custom export options (EXP-05)"
  artifacts:
    ...
    - path: "helm-chart/file-simulator/values-multi-nas.yaml"
      provides: "7 NAS server configurations with unique names, NodePorts, hostPaths, exportOptions"
      min_lines: 150
...
**Export options configurability (EXP-05):**
- Use `$nas.exportOptions` field to configure NFS export options per server
- Default to "rw,sync,no_root_squash" if not specified
- In unfs3 command args, use: `{{ $nas.exportOptions | default "rw,sync,no_root_squash" }}`
- This allows per-NAS customization (e.g., read-only for backup NAS)
...
- Use $nas.exportOptions for configurable export options
  </action>
  <verify>
```bash
...
Template renders without errors. Verify range loop generates resources for each NAS server.
  ...
  <done>nas-multi.yaml template exists, passes Helm lint, uses range loop over .Values.nasServers, supports exportOptions per server</done>
</task>
...
nasServers:
  # Input servers (3) - read-write for file ingestion
  - name: nas-input-1
    enabled: true
    exportOptions: "rw,sync,no_root_squash"  # EXP-05: configurable per server
  ...
  # Backup server (1) - read-only example to demonstrate EXP-05
  - name: nas-backup
    exportOptions: "ro,sync,no_root_squash"  # Read-only backup
    # nodePort: 32153

  # Output servers (3) - read-write for system outputs
  - name: nas-output-1
    exportOptions: "rw,sync,no_root_squash"
    # nodePort: 32154
...
**Export options (EXP-05):**
- Each server has explicit exportOptions field
- Input servers: "rw,sync,no_root_squash" (read-write)
- Backup server: "ro,sync,no_root_squash" (read-only example)
- Output servers: "rw,sync,no_root_squash" (read-write)
...
# Verify exportOptions are rendered
helm template file-sim helm-chart/file-simulator -f helm-chart/file-simulator/values-multi-nas.yaml 2>&1 | grep -E "(rw|ro),sync"
# Should show export options in unfs3 args
```
  </verify>
  <done>values-multi-nas.yaml exists with 7 NAS server configs, unique NodePorts 32150-32156, proper resource limits, and per-server exportOptions (EXP-05)</done>
</task>
...
# Verify exportOptions rendered (EXP-05)
grep -E "(rw|ro),sync,no_root_squash" /tmp/rendered.yaml
```
...
5. Each NAS server has configurable exportOptions (EXP-05)
6. No privileged: true in any NAS deployment
7. Helm lint passes without errors
8. Template uses $ for root scope in include calls (no template errors)
</success_criteria>
